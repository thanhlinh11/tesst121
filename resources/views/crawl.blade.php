...
@php
    $defaultBreadcrumbs = [
        ...
        'Crawler' => backpack_url('plugin/xxvnapi-crawler'),
    ];
    ...
    <script>
        ...
        $('.btn-load').click(function(e) {
            ...
            const fetchApi = async (link, from, to) => {
                isFetching = true;
                const response = await fetch("{{ backpack_url('plugin/-crawler/fetch') }}?" +
                    new URLSearchParams({
                        link,
                        from,
                        to
        ...
        const processMovie = async (slug) => {
            ...
            const response = await fetch("{{ backpack_url('plugin/xxvnapi-crawler/crawl') }}", {
                method: 'POST',
                headers: {
        ...
        $("#movies-get-handle").click(async function() {
            const apiDomain = "{{ config('xxvnapi_crawler.domain', 'https://xxvnapi.com') }}";
            let params = $("select[name=movies-get-params]").find(":selected").val();

            const response = await fetch("{{ backpack_url('plugin/xxvnapi-crawler/get-movies') }}", {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
